...
def get_edge_indices_and_attr(adj):
    '''
    add edge attr (features)
    '''
    edges_list = []
...
def featurize_mol(mol):
    ...
    #pos_matrix = np.array([[conf.GetAtomPosition(k).x, conf.GetAtomPosition(k).y, conf.GetAtomPosition(k).z] for k in range(mol.GetNumAtoms())])
